...
# обводка) из "Оталькованные руды/Области оталькования" — mean_abs_err_pct 4.74%
# (было 14.83% на дефолтах, первый проход калибровки давал 4.92%), цель брифа
# ±3% пока не добита до конца (похоже на предел этой формулы детектора — эксперт
# обводит тальк МЕСТАМИ, не всю площадь, так что и сам эталон немного шумный).
# Если продолжать — см. scripts/calibrate_talc.py --resume-from <report.json>
...
TALC_BRIGHT_EXCLUDE_PERCENTILE = 91  # ярче этого => рудная/светлая фаза, не тальк
TALC_DARK_PERCENTILE = 22            # перцентиль матрицы как уровень нерудного фона (50 = медиана)
TALC_ABS_MARGIN = 33.69               # на столько темнее фона по АБСОЛЮТУ (ловит интерьер больших зон)
TALC_LOCAL_WINDOW = 101              # px, окно оценки локального фона (нечётное)
TALC_LOCAL_MARGIN = 14.09              # на столько темнее ЛОКАЛЬНОГО фона (края/остаточный градиент)
TALC_MAX_TEXTURE = 0.03              # потолок норм. локальной дисперсии (тальк гладкий)
TALC_MIN_BLOB_AREA = 97             # px, отбрасывать мелкие крапинки
...
TALC_CONTRAST_MARGIN = 15.85          # тальк темнее окружающего гангу на столько интенсивностей
