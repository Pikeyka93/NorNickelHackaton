...
`app/main.py` — готовый локальный Streamlit-интерфейс для демо:

- загрузка одного или нескольких снимков (`TIFF / PNG / JPEG / BMP`);
- кэшированный вызов `core.analyze()`, чтобы зум и правки не пересчитывали снимок;
- вердикт сорта с источником (`classifier`, `fallback`, `stub`);
- синяя маска талька с зумом и сравнением с оригиналом;
- таблица метрик: сорт, `talc_pct`, уверенность, источник, время, согласованность;
...
- `docs/p4_interface_presentation.pptx` — готовый черновик презентации для сдачи;
- `notebooks/P4_interface_report.ipynb` — подробный ноутбук по тому, что сделано в P4.
...
docs/                # чек-лист сдачи, сценарий видео, план презентации
